...
def display_recommendations(recommendations):
    ...
    row_number = st.text_input("Row Number:")
    if st.button("Download"):
        try:
            row_number = int(row_number)
            if 1 <= row_number <= len(df):
                link = df.iloc[row_number - 1]['Link']
                link2="https://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data"
                od.download(link2)
                # Hard-coded link for wget
                # wget_command = f"wget {link}"
